...
class StockDAO {
    constructor(id, src, title, text, cardTitle, cardText) {
        this.id = id;
        this.src = src;
        this.title = title;
        this.text = text;
    ...
    static find() {
        ...
        return stocks.map(({id, src, title, text, cardTitle, cardText}) => {
            return new this(id, src, title, text, cardTitle, cardText);
        });
    }
    ...
    static findById(id) {
        ...
        return new this(stock.id, stock.src, stock.title, stock.text, stock.cardTitle, stock.cardText);
    }
    ...
    static delete(id) {
        ...
        return filteredStocks.map(({id, src, title, text, cardTitle, cardText}) => {
            return new this(id, src, title, text, cardTitle, cardText);
        });
    }
    static update(id, stockData) {
        ...
        return new this(updatedStock.id, updatedStock.src, updatedStock.title, updatedStock.text, updatedStock.catdTitle, updatedStock.cardText);
    }
    toJSON() {
        return {
            id: this.id,
